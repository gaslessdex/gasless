...
import { generatedTokenPolicy, mergeTokenEntries } from '../token-registry/generated.js';

export type ServerConfig = ReturnType<typeof loadServerConfig>;
export type OperatingMode = 'devnet' | 'private-mainnet' | 'public-mainnet';
export function isMainnetOperatingMode(mode: string): mode is Exclude<OperatingMode, 'devnet'> { return mode === 'private-mainnet' || mode === 'public-mainnet'; }
...
function requireHttps(name: string, value: string | undefined) {
  ...
  catch { throw new GaslessError('CONFIGURATION_ERROR', 'configuration', `${name} must be a valid HTTPS URL on Mainnet.`); }
}

export function assertPrivateWalletRoles(pilotWallets: string[], settlementAddresses: Array<string | undefined>, payer?: string) {
  if (settlementAddresses.some((value) => !value) || new Set(settlementAddresses).size !== 1) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'Mainnet settlement destinations must use the single configured treasury wallet.');
...
export function assertExpectedKoraPayer(actual: string, expected: string | undefined) {
  if (!expected || actual !== expected) throw new GaslessError('CONFIGURATION_ERROR', 'kora_payer_identity', 'Kora returned an unexpected Mainnet payer.');
}

export function loadServerConfig() {
  const operatingMode = optional('GASLESS_OPERATING_MODE') ?? 'devnet';
  if (!['devnet', 'private-mainnet', 'public-mainnet'].includes(operatingMode)) throw new GaslessError('UNSUPPORTED_NETWORK', 'configuration', 'GASLESS_OPERATING_MODE must be devnet, private-mainnet, or public-mainnet.');
  const mainnet = isMainnetOperatingMode(operatingMode);
  const network = mainnet ? 'mainnet-beta' as const : 'devnet' as const;
  const generated = network === 'mainnet-beta' ? generatedTokenPolicy() : { sendTokens: [], swapTokens: [], recoverMints: [] };
  ...
  const crossChainAuthorizationPublicKey = optional('CROSS_CHAIN_AUTHORIZATION_PUBLIC_KEY');
  ...
  try { if (crossChainAuthorizationPublicKey) new PublicKey(crossChainAuthorizationPublicKey); } catch { throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'CROSS_CHAIN_AUTHORIZATION_PUBLIC_KEY must be a valid Ed25519 public key.'); }
  const crossChainEnabled = optional('CROSS_CHAIN_ENABLED') === 'true';
  const crossChainExecutionWalletAllowlist = (optional('CROSS_CHAIN_EXECUTION_WALLET_ALLOWLIST') ?? '').split(',').map((wallet) => wallet.trim()).filter(Boolean);
  try { crossChainExecutionWalletAllowlist.forEach((wallet) => new PublicKey(wallet)); } catch { throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'CROSS_CHAIN_EXECUTION_WALLET_ALLOWLIST contains an invalid Solana address.'); }
  const crossChainAppFeeBps = integer('CROSS_CHAIN_APP_FEE_BPS', 0);
  const crossChainMaximumInputUsd = optional('CROSS_CHAIN_MAX_INPUT_USD') === undefined ? undefined : Number(optional('CROSS_CHAIN_MAX_INPUT_USD'));
  if (crossChainAppFeeBps > 10_000) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'CROSS_CHAIN_APP_FEE_BPS must be between 0 and 10000.');
  if (crossChainMaximumInputUsd !== undefined && (!Number.isFinite(crossChainMaximumInputUsd) || crossChainMaximumInputUsd <= 0)) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'CROSS_CHAIN_MAX_INPUT_USD must be a positive number.');
  if (crossChainEnabled && !optional('RELAY_API_KEY')) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'RELAY_API_KEY is required when cross-chain quotes are enabled.');
  if (crossChainExecutionWalletAllowlist.length && (operatingMode !== 'private-mainnet' || !crossChainEnabled || crossChainAppFeeBps !== 0 || !optional('CROSS_CHAIN_AUTHORIZATION_SECRET_KEY') || crossChainExecutionWalletAllowlist.some((wallet) => !pilotWalletAllowlist.includes(wallet)))) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'A cross-chain execution allowlist requires private Mainnet, zero App Fee, a signing authorization key, and wallets contained in the pilot allowlist.');
  if (operatingMode === 'public-mainnet' && (!crossChainEnabled || crossChainAppFeeBps !== 0 || !optional('CROSS_CHAIN_AUTHORIZATION_SECRET_KEY') || crossChainExecutionWalletAllowlist.length)) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'Public Mainnet cross-chain execution requires zero App Fee, a signing authorization key, and no wallet allowlist.');
  if (crossChainAppFeeBps > 0 && !optional('RELAY_APP_FEE_RECIPIENT')) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'RELAY_APP_FEE_RECIPIENT is required when a cross-chain App Fee is configured.');
  if (operatingMode === 'public-mainnet' && (!optional('TURNSTILE_SITE_KEY') || !optional('TURNSTILE_SECRET_KEY'))) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'Public Mainnet requires Turnstile browser verification.');
  const localTurnstile = operatingMode === 'devnet' && !process.env.VERCEL && process.env.NODE_ENV !== 'test';
  ...
  if (mainnet && (sendPriceMaxAgeSeconds < 1 || sendPriceMaxAgeSeconds > 300 || swapPriceMaxAgeSeconds < 1 || swapPriceMaxAgeSeconds > 300)) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'Mainnet price maximum age must be between 1 and 300 seconds.');
  ...
  if (mainnet && (!rpcPrimaryUrl || (operatingMode === 'private-mainnet' && !pilotWalletAllowlist.length) || !optional('UPSTASH_REDIS_REST_URL') || !optional('UPSTASH_REDIS_REST_TOKEN') || !optional('SUPABASE_URL') || !optional('SUPABASE_SERVICE_ROLE_KEY') || optional('RELAYER_PROVIDER') !== 'kora' || !optional('KORA_RPC_URL') || !optional('KORA_AUTH_TOKEN') || !koraExpectedPayer || !optional('RECOVER_AUTHORIZATION_SECRET_KEY') || !optional('JUPITER_API_KEY') || !optional('SENTRY_DSN') || settlementAddresses.some((value) => !value) || !generated.sendTokens.length || !generated.swapTokens.length || !generated.recoverMints.length || !optional('SEND_MAXIMUM_SPONSORED_COST_LAMPORTS') || !optional('SWAP_MAXIMUM_SPONSORED_COST_LAMPORTS') || !optional('RECOVER_MAXIMUM_SPONSORED_COST_LAMPORTS') || !optional('MAXIMUM_SPONSORED_COST_LAMPORTS') || !optional('SWAP_MAXIMUM_PRICE_IMPACT_BPS') || !optional('RECOVER_MAX_PRICE_IMPACT_BPS'))) throw new GaslessError('CONFIGURATION_ERROR', 'configuration', 'Mainnet prerequisites are incomplete. RPC, explicit limits, generated token policy, settlement destinations, Redis, Supabase, Jupiter pricing, Sentry, Recover authorization, and authenticated Kora are required.');
  if (mainnet) assertPrivateWalletRoles(pilotWalletAllowlist, settlementAddresses);
  return {
    operatingMode: operatingMode as OperatingMode,
    network,
    ...
    turnstileSiteKey: optional('TURNSTILE_SITE_KEY') ?? (localTurnstile ? '1x00000000000000000000AA' : undefined),
    turnstileSecretKey: optional('TURNSTILE_SECRET_KEY') ?? (localTurnstile ? '1x0000000000000000000000000000000AA' : undefined),
    turnstileAllowedHostnames: (optional('TURNSTILE_ALLOWED_HOSTNAMES') ?? (localTurnstile ? 'localhost,127.0.0.1' : 'gasless.exchange,www.gasless.exchange')).split(',').map((hostname) => hostname.trim().toLowerCase()).filter(Boolean),
    browserVerificationTtlSeconds: integer('BROWSER_VERIFICATION_TTL_SECONDS', 900),
    browserChallengeLimit: integer('BROWSER_CHALLENGE_LIMIT', 10),
    crossChainEnabled,
    crossChainExecutionEnabled: operatingMode === 'public-mainnet' || crossChainExecutionWalletAllowlist.length > 0,
    crossChainPublicExecution: operatingMode === 'public-mainnet',
    crossChainExecutionWalletAllowlist,
    crossChainAppFeeBps,
    crossChainQuoteTtlSeconds: integer('CROSS_CHAIN_QUOTE_TTL_SECONDS', 120),
    crossChainMaximumInputUsd,
    relayApiKey: optional('RELAY_API_KEY'),
    relayAppFeeRecipient: optional('RELAY_APP_FEE_RECIPIENT'),
    rpcPrimaryUrl,
