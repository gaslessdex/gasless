...
import { RAYDIUM_CLMM_DEX, type JupiterBuild, type JupiterRouter, routeFingerprint, wrappedSolAccount } from '../jupiter/service.js';
...
export class RecoverEngine {
  ...
  async createQuote(input: { walletAddress: string; network: SolanaNetwork; tokenAccount: string; clientRequestId: string; requestId: string }) {
    ...
    const preview = await this.jupiter.quote({ inputMint: account.mint, amount: account.tokenAmountRaw, slippageBps: this.policy.slippageBps, dexes: [RAYDIUM_CLMM_DEX], maxPriceImpactBps: this.policy.maxPriceImpactBps });
  ...
  async prepare(quoteId: string, walletAddress: string, requestId: string, deferSigningWindow = false) {
    ...
    const build = await this.buildRoute(recover.account, walletAddress, feePayer);
  ...
  private async buildRoute(account: RecoverAccount, walletAddress: string, feePayer: string) { const build = await this.jupiter.build({ inputMint: account.mint, amount: account.tokenAmountRaw, taker: walletAddress, payer: feePayer, slippageBps: this.policy.slippageBps, dexes: [RAYDIUM_CLMM_DEX], blockhashSlotsToExpiry: this.policy.blockhashSlotsToExpiry }); this.jupiter.validate(build, { inputMint: account.mint, amount: account.tokenAmountRaw, taker: walletAddress, payer: feePayer, slippageBps: this.policy.slippageBps, maxPriceImpactBps: this.policy.maxPriceImpactBps, dexes: [RAYDIUM_CLMM_DEX] }, await this.rpc.getBlockHeight()); return build; }
