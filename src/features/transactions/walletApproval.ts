...
function walletFailureReturnedSignature(error: unknown) { return Boolean(error && typeof error === 'object' && 'userSignatureReturned' in error && (error as WalletSigningError).userSignatureReturned); }
...
export async function awaitWalletApproval({ sign, getBlockHeight, lastValidBlockHeight, now = Date.now, wait = (ms: number) => new Promise<void>((resolve) => window.setTimeout(resolve, ms)), pollMs = WALLET_BLOCK_POLL_MS }: { sign: () => Promise<Uint8Array>; getBlockHeight: () => Promise<number>; lastValidBlockHeight: number; now?: () => number; wait?: (ms: number) => Promise<void>; pollMs?: number }): Promise<WalletApprovalOutcome> {
  ...
  for (;;) {
    if (!signedTransaction) { const result = await Promise.race([walletResult, wait(pollMs).then(() => null)]); if (result?.kind === 'failed') return { status: 'failed', classification: walletFailureClassification(result.error), elapsedMs: now() - startedAt, error: result.error, userSignatureReturned: walletFailureReturnedSignature(result.error) }; if (result?.kind === 'signed') signedTransaction = result.signedTransaction; }
