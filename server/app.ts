...
import { isMainnetOperatingMode, type ServerConfig } from './config/env.js';
...
import { BrowserVerificationService } from './abuse/browser-verification.js';
import { RelayClient } from './relay/client.js';
import { CrossChainService } from './cross-chain/service.js';
import { RelayAuthorizationSigner } from './cross-chain/authorization.js';
import { CrossChainSponsorshipLifecycle } from './cross-chain/lifecycle.js';
...
export function createServices(config: ServerConfig) {
  ...
  const priceProvider = isMainnetOperatingMode(config.operatingMode)
  ...
  if (isMainnetOperatingMode(config.operatingMode) && recoverAuthorizationSigner?.publicKey !== config.recoverAuthorizationPublicKey) throw new GaslessError('CONFIGURATION_ERROR', 'recover_authorization', 'The Recover authorization signing key does not match the configured verification key.');
  ...
  const browserVerification = new BrowserVerificationService(temporary, { siteKey: config.turnstileSiteKey, secretKey: config.turnstileSecretKey, allowedHostnames: config.turnstileAllowedHostnames, ttlSeconds: config.browserVerificationTtlSeconds });
  const relay = new RelayClient(config.relayApiKey);
  const crossChainAuthorizationSigner = config.crossChainAuthorizationSecretKey ? new RelayAuthorizationSigner(config.crossChainAuthorizationSecretKey) : undefined;
  if (config.crossChainExecutionEnabled && crossChainAuthorizationSigner?.publicKey !== config.crossChainAuthorizationPublicKey) throw new GaslessError('CONFIGURATION_ERROR', 'cross_chain_authorization', 'The cross-chain authorization signing key does not match the configured verification key.');
  const reservedSigningRoles = [
    ...config.pilotWalletAllowlist,
    config.claimFeeDestination,
    config.burnFeeDestination,
    config.recoverFeeDestination,
    config.sendReimbursementWallet,
    config.sendServiceFeeWallet,
    config.swapReimbursementWallet,
    config.swapServiceFeeWallet,
    config.koraExpectedPayer,
    recoverAuthorizationSigner?.publicKey,
  ];
  if (crossChainAuthorizationSigner && reservedSigningRoles.includes(crossChainAuthorizationSigner.publicKey)) throw new GaslessError('CONFIGURATION_ERROR', 'cross_chain_authorization', 'The cross-chain authorization key must be separate from pilot, treasury, Kora payer, and Recover authorization roles.');
  const crossChainSponsorship = new CrossChainSponsorshipLifecycle(temporary, durable, relay, relayer, risk, crossChainAuthorizationSigner);
  const crossChain = new CrossChainService(temporary, relay, relayer, rpc, crossChainSponsorship, { enabled: config.crossChainEnabled, publicExecution: config.crossChainPublicExecution, executionWallets: config.crossChainExecutionWalletAllowlist, appFeeRecipient: config.relayAppFeeRecipient, appFeeBps: config.crossChainAppFeeBps, quoteTtlSeconds: config.crossChainQuoteTtlSeconds, maximumInputUsd: config.crossChainMaximumInputUsd, maximumSponsoredCostLamports: config.maximumSponsoredCostLamports });
  return { session: new SessionService(temporary, config.sessionTtlSeconds, config.network), browserVerification, engine: new TransactionEngine(temporary, durable, rpc, relayer, controls, registry, config.quoteTtlSeconds), claim, recover, burn, send, swap, crossChain, crossChainSponsorship, risk, durable, temporary, rpc, relayer, registry, dynamicTokens };
}
