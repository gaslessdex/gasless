...
Object.assign(Rpc.prototype, {
  async getMultipleAccounts(this: Rpc, addresses: string[]) { return addresses.map((address) => this.accounts.get(address) ?? null); },
  async getTokenAccountsByOwner(this: Rpc, owner: string, programId: string) { return [...this.accounts].filter(([, account]) => account.owner === programId && new PublicKey(Buffer.from(account.data[0], 'base64').subarray(32, 64)).toBase58() === owner).map(([pubkey, account]) => ({ pubkey, account })); },
});
