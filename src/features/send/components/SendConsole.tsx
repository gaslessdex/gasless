...
import { sendTokenCatalog } from '../../../config/solanaTokenCatalog';
...
function catalogSendToken(mint: string): SendToken | undefined { const token = sendTokenCatalog.find((item) => item.mint === mint); return token ? { mint: token.mint, symbol: token.symbol, name: token.name, image: token.image, decimals: token.decimals, tokenProgram: token.tokenProgram, balanceRaw: '0', sourceAccount: '' } : undefined; }
...
function SolanaSendPanel({ connected, onConnect }: { connected: boolean; onConnect: () => void }) {
  ...
  const tokens = useMemo<TokenOption[]>(() => view.discovery ? view.discovery.tokens.map((item) => ({ id: item.sourceAccount, mint: item.mint, program: item.tokenProgram, symbol: item.symbol, name: item.name ?? 'Approved token', image: item.image, balance: display(item.balanceRaw, item.decimals, item.uiMultiplier), eligible: true })) : sendTokenCatalog.map((item) => ({ id: item.mint, mint: item.mint, program: item.tokenProgram, symbol: item.symbol, name: item.name, image: item.image, eligible: true })), [view.discovery]);
  ...
  useEffect(() => { if (!view.discovery) return; setToken((current) => current ? view.discovery!.tokens.find((item) => item.mint === current.mint) ?? current : current); }, [view.discovery]);
  ...
  return <div className="feature-body send-body">
    <div className="send-form-field send-token-field"><div className="field-heading"><span>TOKEN</span>{token && <span>BALANCE {view.discovery ? display(token.balanceRaw, token.decimals, token.uiMultiplier) : 'LOADING'} {token.symbol}</span>}</div><TokenSelector label="Token to send" value={selectedToken} tokens={tokens} disabled={selectorLocked} onChange={(option) => { const selected = view.discovery?.tokens.find((item) => item.mint === option.mint) ?? catalogSendToken(option.mint); setAccountNotice(undefined); setToken(selected); setMax(false); invalidatePreview(); }} /></div>
    <div className="send-form-field"><div className="field-heading"><label htmlFor="send-amount">AMOUNT</label></div><div className="amount-row send-amount-row"><input id="send-amount" inputMode="decimal" value={max && quoted ? display(quoted.recipientAmountRaw, quoted.token.decimals, quoted.token.uiMultiplier) : amount} disabled={busy} onChange={(event) => { setAccountNotice(undefined); setAmount(event.target.value); setMax(false); invalidatePreview(); }} placeholder="0.00" aria-invalid={Boolean(amountError)} aria-describedby={amountError ? 'send-amount-error' : undefined} /><button type="button" disabled={!token?.balanceRaw || busy} onClick={() => { setAccountNotice(undefined); setMax(true); setAmount(''); invalidatePreview(); }}>MAX</button></div><FieldError id="send-amount-error">{amountError}</FieldError></div>
    <label className="recipient-field" htmlFor="send-recipient"><span>RECIPIENT WALLET</span><input id="send-recipient" value={recipient} disabled={busy} onChange={(event) => { setAccountNotice(undefined); setRecipient(event.target.value.trim()); invalidatePreview(); }} placeholder="Solana wallet address" autoComplete="off" autoCapitalize="none" autoCorrect="off" spellCheck={false} aria-invalid={Boolean(recipientError)} aria-describedby={recipientError ? 'send-recipient-error' : undefined} /></label><FieldError id="send-recipient-error">{recipientError}</FieldError>
    <GaslessStatus connected={connected} /><div className="summary-line"><span>RECIPIENT RECEIVES</span><strong>{quoted ? `${display(quoted.recipientAmountRaw, quoted.token.decimals, quoted.token.uiMultiplier)} ${symbol}` : max ? `MAX ${symbol}` : amount && !amountError ? `${amount} ${symbol}` : '—'}</strong></div>
    ...
    <DetailSection><DetailRow label="You send" value={quoted ? `${display(quoted.recipientAmountRaw, quoted.token.decimals, quoted.token.uiMultiplier)} ${symbol}` : '—'} /><DetailRow label="Recipient receives" value={quoted ? `${display(quoted.recipientAmountRaw, quoted.token.decimals, quoted.token.uiMultiplier)} ${symbol}` : '—'} /><DetailRow label="Network cost" value={quoted ? `${display(quoted.sponsorReimbursementRaw, quoted.token.decimals, quoted.token.uiMultiplier)} ${symbol}` : '—'} /><DetailRow label="GASLESS fee (0.10%, max $1)" value={quoted ? `${display(quoted.serviceFeeRaw, quoted.token.decimals, quoted.token.uiMultiplier)} ${symbol}` : '—'} /><DetailRow label="Total from wallet" value={quoted ? `${display(quoted.totalDebitRaw, quoted.token.decimals, quoted.token.uiMultiplier)} ${symbol}` : '—'} /><p>The recipient amount, sponsored network cost, and GASLESS fee settle atomically. SOL needed from you: 0.</p></DetailSection>
...
export function SendConsole({ connected, onConnect }: { connected: boolean; onConnect: () => void }) {
  ...
  return <>
    <div className="console-mode-tabs" role="tablist" aria-label="Send mode">
      <button id="send-tab" type="button" role="tab" aria-selected={tab === 'send'} aria-controls="send-panel" className={tab === 'send' ? 'is-active' : ''} onClick={() => setTab('send')}>SEND</button>
      <button id="bridge-tab" type="button" role="tab" aria-selected={tab === 'bridge'} aria-controls="bridge-panel" className={tab === 'bridge' ? 'is-active' : ''} onClick={() => setTab('bridge')}>BRIDGE</button>
    </div>
    <div id="send-panel" role="tabpanel" aria-labelledby="send-tab" hidden={tab !== 'send'}><SolanaSendPanel connected={connected} onConnect={onConnect} /></div>
    <div id="bridge-panel" role="tabpanel" aria-labelledby="bridge-tab" hidden={tab !== 'bridge'}><CrossChainPanel entryMode="bridge" /></div>
  </>;
}
