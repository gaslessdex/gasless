...
export class SendEngine {
  ...
  async listTokens() { return (await this.activeEntries()).map(({ mint, symbol, decimals }) => ({ mint, symbol, decimals })); }
  ...
  async createQuote(input: { walletAddress: string; network: SolanaNetwork; mint: string; recipient: string; amount?: string; max?: boolean; clientRequestId: string; requestId: string }) {
    ...
    const entry = await this.priced(evaluated.entry); this.assertEntry(entry); const feePayer = await this.relayer.getFeePayerPublicKey(); const recipient = this.validateRecipient(input.recipient, input.walletAddress, feePayer);
    ...
    try { if (destinationInfo) validateDestinationAccount(destinationInfo, recipient, entry.mint, destinationAccount); } catch { throw new GaslessError('INVALID_REQUEST', 'send_recipient', 'This address cannot safely receive Gasless Send.'); }
    ...
    for (const [address, owner] of [[reimbursementDestination, this.policy.reimbursementWallet!], [serviceFeeDestination, this.policy.serviceFeeWallet!]] as const) { try { validateDestinationAccount(await this.rpc.getAccountInfo(address), owner, entry.mint, address); } catch { throw new GaslessError('CONFIGURATION_ERROR', 'send_configuration', 'GASLESS Send settlement accounts are not configured for this token.'); } }
    const max = input.max === true; const amountRaw = max ? 0n : parseAmount(input.amount ?? '', entry.decimals);
    ...
    const ataRent = send.recipientAtaExists ? 0n : BigInt(await this.rpc.getMinimumBalanceForRentExemption(165));
    const preview = await prepareSendTransaction({ quote, send, feePayer, rpc: this.rpc, ataRentLamports: ataRent, tokenUsdPriceMicros: BigInt(entry.usdPriceMicros!), solUsdPriceMicros: BigInt(entry.solUsdPriceMicros!), reimbursementBufferBps: entry.reimbursementBufferBps ?? 0, serviceFeeBps: this.policy.serviceFeeBps, serviceFeeCapUsdMicros: BigInt(this.policy.serviceFeeCapUsdMicros), maximumSponsoredCostLamports: BigInt(this.policy.maximumSponsoredCostLamports) });
  ...
  async prepare(quoteId: string, walletAddress: string, requestId: string, requestReceivedAt = Date.now(), deferSigningWindow = false) {
    ...
    const [evaluated, currentSource, currentDestination, reimbursementAccount, serviceFeeAccount, feePayer, rent] = await Promise.all([
      ...
      timed(() => send.recipientAtaExists ? Promise.resolve(0) : this.rpc.getMinimumBalanceForRentExemption(165), (duration) => { rentMs = duration; }),
    ]);
    ...
    const pricingStarted = Date.now(); if (!send.pricing) throw this.refreshRequired(); const entry = { ...evaluated.entry, usdPriceMicros: send.pricing.tokenUsdPriceMicros, solUsdPriceMicros: send.pricing.solUsdPriceMicros, priceUpdatedAt: send.pricing.observedAt }; const pricingCompletedAt = new Date().toISOString(); const pricingMs = Date.now() - pricingStarted; this.assertEntry(entry);
    ...
    try { if (currentDestination) validateDestinationAccount(currentDestination, send.recipientWallet, send.token.mint, send.destinationAccount); }
    catch { throw new GaslessError('INVALID_REQUEST', 'send_recipient', 'The recipient account state changed. Review the Send again.'); }
    try {
      validateDestinationAccount(reimbursementAccount, this.policy.reimbursementWallet!, send.token.mint, send.reimbursementDestination);
      validateDestinationAccount(serviceFeeAccount, this.policy.serviceFeeWallet!, send.token.mint, send.serviceFeeDestination);
  ...
  private async activeEntries() { const entries = await this.registry.list('SEND'); return entries.filter((entry) => { try { this.assertIdentity(entry); return true; } catch { return false; } }); }
  ...
  private assertIdentity(entry: TokenRegistryEntry) { new PublicKey(entry.mint); new PublicKey(entry.tokenProgram); if (!this.policy.reimbursementWallet || !this.policy.serviceFeeWallet) throw new Error(); new PublicKey(this.policy.reimbursementWallet); new PublicKey(this.policy.serviceFeeWallet); if (entry.extensions.length || entry.tokenProgram !== 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA' || !Number.isInteger(entry.decimals) || entry.decimals < 1 || entry.decimals > 9) throw new Error(); }
  ...
  private async priced(entry: TokenRegistryEntry) { if (!this.prices) return entry; const prices = await this.prices.getUsdPrices([entry.mint]); await this.prices.assertExecutablePriceConfidence?.(entry.mint, entry.decimals, prices); return { ...entry, usdPriceMicros: prices[entry.mint]?.usdPriceMicros, solUsdPriceMicros: prices[WRAPPED_SOL_MINT]?.usdPriceMicros, priceUpdatedAt: [prices[entry.mint]?.observedAt, prices[WRAPPED_SOL_MINT]?.observedAt].sort()[0] }; }
  private async recheck(send: SendQuoteDetails, wallet: string) { const { entry, decision } = await this.registry.evaluate('SEND', send.token.mint); if (decision !== 'supported' || !entry) throw new GaslessError('TOKEN_UNSUPPORTED', 'send_token', 'This token is temporarily unavailable.'); this.assertIdentity(entry); const source = inspectSendTokenAccount(send.token.sourceAccount, await this.rpc.getAccountInfo(send.token.sourceAccount), wallet, entry); if (!source || BigInt(source.balanceRaw) < BigInt(send.totalDebitRaw)) throw new GaslessError('TOKEN_UNSUPPORTED', 'send_balance', `You need a little more ${send.token.symbol} to cover the amount, GASLESS fee, and sponsored network cost.`); const destination = await this.rpc.getAccountInfo(send.destinationAccount); if (Boolean(destination) !== send.recipientAtaExists) throw new GaslessError('MESSAGE_MISMATCH', 'send_state_recheck', 'The recipient token account changed. Refresh the Send preview.'); if (destination) { try { validateDestinationAccount(destination, send.recipientWallet, send.token.mint, send.destinationAccount); } catch { throw new GaslessError('MESSAGE_MISMATCH', 'send_state_recheck', 'The recipient token account changed. Refresh the Send preview.'); } } }
  private async resumeSubmitted(quote: TransactionQuote, alreadyCompleted = true) { const send = quote.send!; const confirmation = await this.confirm(send.signature!, send.prepared!.lastValidBlockHeight); const reconciliation = await this.reconcile(quote, confirmation); send.reconciliation = reconciliation; send.status = reconciliation.status === 'confirmed' ? 'reconciled' : reconciliation.status === 'failed' ? 'failed' : 'submitted'; quote.status = send.status; await this.persistQuote(quote); await this.durable.updateIntentStatus(quote.intent.intentId, quote.status); if (reconciliation.status !== 'confirmed') throw new GaslessError('RECONCILIATION_FAILED', 'reconciliation', 'This Send was submitted and is still being verified.', true); return { transactionId: send.prepared!.transactionId, signature: send.signature!, confirmation, reconciliation, alreadyCompleted }; }
  ...
  private persistQuote(quote: TransactionQuote) { const ttl = quote.status === 'reconciled' ? 86_400 : quote.send?.prepared ? 300 : Math.max(1, Math.ceil((Date.parse(quote.expiresAt) - Date.now()) / 1000)); return this.temporary.saveQuote(quote, ttl); }
}
